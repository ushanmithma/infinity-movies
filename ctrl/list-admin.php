...
$admin_count = "SELECT COUNT(id) AS count FROM admins";
$admin_count_result_set = mysqli_query($connection, $admin_count);
$admin_count_result = mysqli_fetch_assoc($admin_count_result_set);

$admins_members_count = $admin_count_result['count'];

$rows_pre_page = 12;

$last_page = ceil($admins_members_count / $rows_pre_page);
...
$query = "SELECT * FROM admins LIMIT {$start}, {$rows_pre_page}";
...
// first page
$first = "<a href=\"list-admin.php?p=1\">First</a>";

// last page
$last_page_no = ceil($admins_members_count / $rows_pre_page);
$last = "<a href=\"list-admin.php?p={$last_page_no}\">Last</a>";
...
if ($page_no >= $last_page_no) {
	...
	$next = "<a href=\"list-admin.php?p={$next_page_no}\">Next</a>";
}
...
if ($page_no <= 1) {
	...
	$prev = "<a href=\"list-admin.php?p={$prev_page_no}\">Previous</a>";
}
?>
<!DOCTYPE html>
<html lang="en">
		...
		<table>
			...
			while ($row = mysqli_fetch_assoc($result_set)) {
				...
				if ($current_id == $id || $id == 1) {
					echo '<tr><td>'.$id.'</td><td>'.$name.'</td><td>'.$username.'</td><td>'.$last_seen.'</td><td><a href="update-admin.php?id='.$id.'">Update</a></td><td></td></tr>';
				} else {
					echo '<tr><td>'.$id.'</td><td>'.$name.'</td><td>'.$username.'</td><td>'.$last_seen.'</td><td><a href="update-admin.php?id='.$id.'">Update</a></td><td><a href="delete-admin.php?id='.$id.'&p='.$page_no.'">Delete</a></td></tr>';
				}
